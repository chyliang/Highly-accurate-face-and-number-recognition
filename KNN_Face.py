...
directory = './extracted_face_training_data_face'

x_train = []
# get x, and construct training set
for f in range(451):
    data = np.genfromtxt(directory + '/face_%d' % f, dtype=float)
    x_train.append(data.reshape(60*70))
...
x_test = []
for f in range(150):
    data_test = np.genfromtxt("./extracted_face_test_data/face_%d" % f, dtype=float)
    x_test.append(data_test.reshape(60*70))
...
random_indexes = random.sample([i for i in range(451)],int(451*0.6))
